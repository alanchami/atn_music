...
fetch('https://cors-anywhere.herokuapp.com/https://api.deezer.com/search?q='+busqueda)
    ...
    .then(function(respuesta){
      ...
      for (const resultado of tracks) {
        ...
        nombre.innerHTML += `<a href="detalles-track.html?id=${resultado.id}"><img class="inolvidable" src="${resultado.album.cover_xl}" alt="">
        <h5>${resultado.type}</h5>
        <h3>${resultado.artist.name}</h3>
        <h3>${resultado.album.title}</h3></a>`
